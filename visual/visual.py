...
def plotday(data, label):
    ...
    for i in range(len(txt)):
        plt.text(peak_hour[i] - 5, point[i] + 10, txt[i], weight="bold")
    plt.xticks(list(range(13, 24 * 7, 24)), label)
    plt.savefig("./day.png", dpi=500)
    plt.show()
...
def plot_rmse_mae(dataset, MC, DMC, Y_S, Y_E, savefig):
    ...
    for i in range(len(MC)):
        plt.text(bar1[i] - 0.14, MC[i] + 0.1, MC[i], weight="bold")
        plt.text(bar2[i] - 0.1, DMC[i] + 0.1, DMC[i], weight="bold")
